...
class VoiceService : Service() {
    ...
    fun startVoiceService(settings: VoXNetSettings) {
        ...
        webSocketClient = WebSocketClient(settings).apply {
                ...
                when (message.type) {
                    MessageTypes.STATUS -> {
                        val state = when (message.state) {
                            ...
                            StatusStates.SMS_SENT -> {
                                onSMSResult?.invoke(true, "SMS sent successfully")
                                CallState.DISCONNECTED
                            }
                            StatusStates.SMS_FAIL -> {
                                onSMSResult?.invoke(false, "SMS failed to send")
                                CallState.DISCONNECTED
                            }
            ...
            onAudioFrame = { frame ->
                audioManager?.queueAudioFrame(frame)
            }
        }
        
        audioManager?.onAudioFrame = { frame ->
            webSocketClient?.sendAudioFrame(frame)
        }
    ...
    fun makeCall(number: String) {
        webSocketClient?.sendControlMessage(
            ControlMessage(MessageTypes.CALL_REQ, number = number)
        )
    ...
    fun endCall() {
        webSocketClient?.sendControlMessage(
            ControlMessage(MessageTypes.END)
        )
    }
    
    fun sendSMS(number: String, text: String) {
        webSocketClient?.sendControlMessage(
            ControlMessage(MessageTypes.SMS_REQ, number = number, text = text)
        )
    }
    
    fun sendSOS() {
        webSocketClient?.sendControlMessage(
            ControlMessage(MessageTypes.SOS)
        )
    }
    
    fun toggleSpeaker(useSpeaker: Boolean) {
        audioManager?.toggleSpeaker(useSpeaker)
    }
    
    private fun createNotificationChannel() {
        val channel = NotificationChannel(
            "VOICE_SERVICE_CHANNEL",
            "VoXNet Voice Service",
